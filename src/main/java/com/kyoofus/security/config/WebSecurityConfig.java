...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterStaticResources(HttpSecurity http) throws Exception {
        ...
        // 정적자원만 처리
        http.securityMatcher(AUTH_STATIC_WHITELIST).authorizeHttpRequests(auth -> auth
           .requestMatchers(AUTH_STATIC_WHITELIST).permitAll()  // working well 
    ...
    public SecurityFilterChain filterMain(HttpSecurity http) throws Exception {
        ...
        // http.securityMatcher(AUTH_WHITELIST).authorizeHttpRequests(auth -> auth
        //     .anyRequest().authenticated()
        // );



        // SpringBoot3, SpringSecurity 6.1에서는 아래 설정이 작동하지 않는다. 
        // http.authorizeHttpRequests(auth -> auth
        //     .requestMatchers(AUTH_WHITELIST).permitAll()  // is not working 
        //     // anyRequest()를 사용하면 Can't configure anyRequest after itself 오류 발생한다. 
